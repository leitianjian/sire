...
auto SireGetDebug::prepareNrt() -> void {
  std::cout << "execute get_debug" << std::endl;
  option() |=
  ...
  if (part_pq_only != cmdParams().end()) {
    simulator.getModelState(
        [](aris::server::ControlServer& cs, simulator::SimulatorBase& s,
           std::any& data) -> void {
          auto& get_param = std::any_cast<GetDebugParam&>(data);
  ...
  auto out_data = std::any_cast<GetDebugParam&>(param);
  auto& inter =
...
auto SireGetDebug::collectNrt() -> void {}

SireGetDebug::SireGetDebug(const std::string& name) {
  aris::core::fromXmlString(command(),
                            "<Command name=\"get_debug\">"
                            "	<GroupParam>"
...
ARIS_REGISTRATION {
  aris::core::class_<SireGetDebug>("SireGetDebug").inherit<aris::plan::Plan>();
}
}  // namespace sire::plan