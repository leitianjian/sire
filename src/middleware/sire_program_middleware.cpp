...
struct SireProgramMiddleware::Imp {
...
auto SireProgramMiddleware::lastError() -> std::string {
  return imp_->last_error_;
}
auto SireProgramMiddleware::lastErrorCode() -> int {
  return imp_->last_error_code_;
}
auto SireProgramMiddleware::lastErrorLine() -> int {
  return imp_->last_error_line_;
}
auto SireProgramMiddleware::isAutoMode() -> bool { return imp_->is_auto_mode_; }
auto SireProgramMiddleware::isAutoRunning() -> bool {
  return imp_->auto_thread_.joinable();
}
auto SireProgramMiddleware::isAutoPaused() -> bool {
  return imp_->is_pause_.load();
}
auto SireProgramMiddleware::isAutoStopped() -> bool {
  return imp_->is_stop_.load();
}
auto SireProgramMiddleware::currentFileLine() -> std::tuple<std::string, int> {
  std::unique_lock<std::mutex> lck(imp_->auto_mu_);
  return std::make_tuple(imp_->current_file_, imp_->current_line_);
}
auto SireProgramMiddleware::executeCmd(
    std::string_view str, std::function<void(std::string)> send_ret,
  ...
  if (cmd == "program") {
    ...
    for (auto& [param, value] : params) {
      if (param == "set_auto") {
        ...
        if (isAutoRunning()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not set manual when auto running");
        } else {
        ...
        if (isAutoRunning()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not set content when auto running");
        } else {
          ...
          try {
            // imp_->calculator_ =
            // aris::server::ControlServer::instance().model().calculator();
            ...
            for (auto& node : js) {
              files[node["name"].get<std::string>()] =
                  node["content"].get<std::string>();
            ...
            for (auto& str : imp_->language_parser_.varPool()) {
              ...
              auto trim_left = [](std::string_view& input,
                ...
                return point == std::string::npos
                           ? std::string_view()
                           : input.substr(point, std::string::npos);
              };

              std::string_view input = str;
              if (auto var = cut_str(input, " "); var.empty())
                THROW_FILE_LINE(
                    "invalid command string: please at least contain a word");
              ...
              auto value = input;
              c.addVariable(name, type,
                            c.calculateExpression(std::string(type) + "(" +
                                                  std::string(value) + ")")
                                .second);
            }
        ...
        if (!isAutoMode()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not goto in manual mode");
        } else if (isAutoRunning()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not goto when running");
        } else {
          try {
            auto file = value.substr(0, value.find_first_of("."));
            auto line = std::stoi(
                std::string(value.substr(value.find_first_of(".") + 1)));

            imp_->language_parser_.gotoFileLine(std::string(file) + ".aris",
                                                line);
        ...
        if (!isAutoMode()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not foward in manual mode");
        } else if (isAutoRunning()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not foward when running");
          ...
          std::swap(imp_->calculator_,
                    dynamic_cast<aris::dynamic::Model&>(
                        ...
                        .calculator());
          auto& c = dynamic_cast<aris::dynamic::Model&>(
                        aris::server::ControlServer::instance().model())
                        .calculator();
          ...
          if (imp_->language_parser_.isEnd()) {
            ...
            ARIS_PRO_COUT << imp_->language_parser_.currentLine() << "---"
                          << imp_->language_parser_.currentCmd() << std::endl;
            // LOG_INFO << "pro " << imp_->language_parser_.currentLine() <<
            // "---" << imp_->language_parser_.currentCmd() << std::endl;
            if (imp_->language_parser_.currentWord() == "if" ||
                imp_->language_parser_.currentWord() == "while") {
              try {
                auto ret = c.calculateExpression(
                    imp_->language_parser_.currentParamStr());

                if (auto ret_double = std::any_cast<double>(&ret.second)) {
                  imp_->language_parser_.forward(*ret_double != 0.0);
                  std::unique_lock<std::mutex> lck(this->imp_->auto_mu_);
                  imp_->current_line_ = imp_->language_parser_.currentLine();
                } else if (auto ret_mat =
                               std::any_cast<aris::core::Matrix>(&ret.second)) {
                  ...
                  ARIS_PRO_COUT << imp_->last_error_line_
                                << "---err_code:" << imp_->last_error_code_
                                << "  err_msg:" << imp_->last_error_
                                << std::endl;
                  break;
                }
              } catch (std::exception& e) {
                ...
                ARIS_PRO_COUT << imp_->last_error_line_
                              << "---err_code:" << imp_->last_error_code_
                              << "  err_msg:" << imp_->last_error_ << std::endl;
                break;
              }
            } else {
            ...
            ARIS_PRO_COUT << imp_->language_parser_.currentLine() << "---"
                          << imp_->language_parser_.currentCmd() << std::endl;
            ARIS_LOG(aris::core::LogLvl::kError, 0, {"pro %5d---%s"},
                     imp_->language_parser_.currentLine(),
                     imp_->language_parser_.currentCmd());
            try {
              ...
              ARIS_PRO_COUT << imp_->last_error_line_
                            << "---err_code:" << imp_->last_error_code_
                            << "  err_msg:" << imp_->last_error_ << std::endl;
              ARIS_LOG(aris::core::LogLvl::kError, 0,
                       {"pro %5d---err_code:%s  err_msg:"},
                       imp_->last_error_line_, imp_->last_error_code_,
                       imp_->last_error_);
            }
          } else {
            ...
            auto ret = cs.executeCmdInCmdLine(
                cmd,
                [&, current_line, next_line](aris::plan::Plan& plan) -> void {
                  std::unique_lock<std::mutex> lck(imp_->auto_mu_);
                  imp_->current_line_ = next_line;
                });

            ARIS_PRO_COUT << current_line << "---" << ret->cmdId() << "---"
                          << ret->cmdString() << std::endl;
            // LOG_INFO << "pro " << current_line << "---" << ret->cmdId() <<
            // "---" << ret->cmdString() << std::endl;
            ...
            if (ret->executeRetCode() == aris::plan::Plan::PREPARE_CANCELLED ||
              ...
              ARIS_PRO_COUT << imp_->last_error_line_ << "---" << ret->cmdId()
                            << "---err_code:" << imp_->last_error_code_
                            << "  err_msg:" << imp_->last_error_ << std::endl;
              // LOG_ERROR << "pro " << imp_->last_error_line_ << "---" <<
              // ret->cmdId() << "---err_code:" << imp_->last_error_code_ << "
              // err_msg:" << imp_->last_error_ << std::endl;
            }
          }
        ...
        if (!isAutoMode()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not start program in manual mode");
        } else if (isAutoRunning()) {
          ...
          imp_->auto_thread_ = std::thread([&]() -> void {
            ...
            lck.unlock();
            std::vector<
                std::pair<std::string, std::function<void(aris::plan::Plan&)>>>
                cmd_vec;
            std::vector<int> lines;

            for (int has_error{0};
                 has_error == 0 && (!imp_->language_parser_.isEnd());) {
              ...
              auto server_execute = [&]() -> int {
                ...
                for (int i = 0; i < plans.size(); ++i) {
                  ARIS_PRO_COUT << lines[i] << "---" << plans[i]->cmdId()
                                << "---" << plans[i]->cmdString() << std::endl;
                  // LOG_INFO << "pro " << lines[i] << "---" <<
                  // plans[i]->cmdId() << "---" << plans[i]->cmdString() <<
                  // std::endl;
                }
                cs.waitForAllCollection();
                for (int i = 0; i < plans.size(); ++i) {
                  ...
                  if (plans[i]->executeRetCode() ==
                    ...
                    ARIS_PRO_COUT
                        ...
                        << "  err_msg:" << imp_->last_error_ << std::endl;
                    // LOG_ERROR << "pro " << imp_->last_error_line_ << "---" <<
                    // plans[i]->cmdId() << "---err_code:" <<
                    // imp_->last_error_code_ << "  err_msg:" <<
                    // imp_->last_error_ << std::endl;
                    has_error = -1;
                  }
                }
                cmd_vec.clear();
              ...
              if (imp_->language_parser_.isCurrentLineKeyWord()) {
                ...
                if (imp_->language_parser_.currentWord() == "if" ||
                  ...
                  try {
                    ...
                    if (auto ret_double = std::any_cast<double>(&ret.second)) {
                      ...
                      imp_->current_line_ =
                          imp_->language_parser_.currentLine();
                    } else if (auto ret_mat = std::any_cast<aris::core::Matrix>(
                                   &ret.second)) {
                      imp_->language_parser_.forward(ret_mat->toDouble() !=
                                                     0.0);
                      std::unique_lock<std::mutex> lck(this->imp_->auto_mu_);
                      imp_->current_line_ =
                          imp_->language_parser_.currentLine();
                    } else {
                      imp_->last_error_code_ =
                          aris::plan::Plan::PROGRAM_EXCEPTION;
                      imp_->last_error_ = "invalid expresion";
                      imp_->last_error_line_ =
                          imp_->language_parser_.currentLine();
                      ARIS_PRO_COUT << imp_->last_error_line_
                                    << "---err_code:" << imp_->last_error_code_
                                    << "  err_msg:" << imp_->last_error_
                ...
                try {
                  ...
                  ARIS_PRO_COUT << imp_->last_error_line_
                                ...
                                << std::endl;
                  // LOG_ERROR << "pro " << imp_->last_error_line_ <<
                  // "---err_code:" << imp_->last_error_code_ << "  err_msg:" <<
                  // imp_->last_error_ << std::endl;
                  has_error = -1;
                }
              } else {
                ...
                cmd_vec.push_back(
                    std::pair<std::string,
                              std::function<void(aris::plan::Plan&)>>(
                        cmd,
                        [&, current_line,
                         next_line](aris::plan::Plan& plan) -> void {
            ...
            // aris::server::ControlServer::instance().model().calculator());
            ARIS_PRO_COUT << "---"
                          << (imp_->is_stop_.load() ? "program stopped"
                                                    : "program finished")
                          << std::endl;
        ...
        if (!isAutoMode()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not stop program in manual mode");
        } else {
        ...
        if (!isAutoMode()) {
          ...
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not stop program when not running");
        } else {
        ...
        if (isAutoRunning()) {
          return send_code_and_msg(aris::plan::Plan::PROGRAM_EXCEPTION,
                                   "can not clear error when running");
        } else {
          imp_->last_error_.clear();
          imp_->last_error_code_ = 0;
          imp_->last_error_line_ = 0;
...
SireProgramMiddleware::SireProgramMiddleware(SireProgramMiddleware&& other) =
    default;
SireProgramMiddleware& SireProgramMiddleware::operator=(
    SireProgramMiddleware&& other) = default;
...
ARIS_REGISTRATION {
  ...
  aris::core::class_<SireProgramMiddleware>("SireModuleProgramMiddleware")
      ...
      .prop("modules_pool", &SireProgramMiddleware::resetModulesPool,
            ModulesPoolFunc(&SireProgramMiddleware::modulesPool));
}
}  // namespace sire::middleware