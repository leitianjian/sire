...
HeightField::HeightField(double x_dim, double y_dim, int nrow, int ncol,
    ...
    : CollisionAdapter(part_id, is_dynamic, prt_pm, visible, material, propStr,
                       x_dim, y_dim, nrow, ncol, min_height, heights) {}
...
ARIS_REGISTRATION {
  ...
  auto getNRow = [](HeightField* geo) -> int { return geo->typedShape.nrow(); };
  auto setNCol = [](HeightField* geo, int ncol) -> void {
    geo->typedShape.setNCol(ncol);
  };
  auto getNCol = [](HeightField* geo) -> int { return geo->typedShape.ncol(); };
