...
auto ProgramMiddleware::lastErrorCode() -> int {
  return imp_->last_error_code_;
}
auto ProgramMiddleware::lastErrorLine() -> int {
  return imp_->last_error_line_;
}
auto ProgramMiddleware::isAutoMode() -> bool { return imp_->is_auto_mode_; }
auto ProgramMiddleware::isAutoRunning() -> bool {
  return imp_->auto_thread_.joinable();
}
auto ProgramMiddleware::isAutoPaused() -> bool {
  return imp_->is_pause_.load();
}
auto ProgramMiddleware::isAutoStopped() -> bool {
...
auto ProgramMiddleware::executeCmd(std::string_view str,
...
ProgramMiddleware& ProgramMiddleware::operator=(ProgramMiddleware&& other) =
    default;
