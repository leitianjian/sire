...
auto fetchInterfaceConfig() -> std::string {
  ...
  if (!std::filesystem::exists(interface_path)) {
    ...
    const std::string default_interface_str =
        ...
        "unityUpdateInterval=\"100\"></WebSocket>\n"
        "<LayoutConfig cols=\"48\" rowHeight=\"36\" margin=\"3\" "
        "containerPadding=\"0\" theme=\"dark\"></LayoutConfig>\n"
        "</Interface>\n";
  ...
  my_json js;
  for (auto ele = doc.RootElement()->FirstChildElement(); ele;
    ...
    if (ele->Name() == std::string("Dashboard")) {
      ...
      for (auto e1 = ele->FirstChildElement(); e1;
        ...
        my_json
            j2;  //{"name":"Ethercat配置","type":"EthercatConfiguration","i":"EMlxGXxpwDGgz","w":48,"h":23,"x":0,"y":0,"options":"{}"}
      ...
      js["ws"]["commandSendInterval"] =
          ele->IntAttribute("commandSendInterval");
      js["ws"]["commandSendDelay"] = ele->IntAttribute("commandSendDelay");
      js["ws"]["getInterval"] = ele->IntAttribute("getInterval");
      js["ws"]["unityUpdateInterval"] =
          ele->IntAttribute("unityUpdateInterval");
      ...
      js["layoutConfig"]["containerPadding"] =
          ele->IntAttribute("containerPadding");
...
auto updateDashboard(std::string dash_id, std::string js_str) -> std::string {
  ...
  for (auto js2 : js["cells"]) {
    ...
    new_cell->SetAttribute("options",
                           js2["options"].get<std::string>().c_str());
...
auto deleteCell(std::string dash_id, std::string cell_id) -> std::string {
  ...
  for (auto ele = dash_ele->FirstChildElement(); ele;
       ele = ele->NextSiblingElement()) {
    std::cout << ele->Name() << " " << ele->Attribute("id") << "  " << cell_id
              << std::endl;
      ...
      for (auto& file : std::filesystem::directory_iterator(dir)) {
        if (file.is_regular_file()) {
          if (file.path().extension() == ".dat") {
            ...
            file_js["path"] = "/program/" + dir.path().filename().string() +
            ...
            addVariable = [&](tinyxml2::XMLElement* blk) -> void {
              if (blk->Attribute("type", "jointtarget_variable")) {
                file_js["jointtargets"].push_back(
                    blk->FirstChildElement("field")->GetText());
              } else if (blk->Attribute("type", "robtarget_variable")) {
                file_js["robtargets"].push_back(
                    blk->FirstChildElement("field")->GetText());
              } else if (blk->Attribute("type", "speed_variable")) {
                file_js["speeds"].push_back(
                    blk->FirstChildElement("field")->GetText());
              } else if (blk->Attribute("type", "zone_variable")) {
                file_js["zones"].push_back(
                    blk->FirstChildElement("field")->GetText());
              }

              if (auto next = blk->FirstChildElement("next"))
                addVariable(next->FirstChildElement());
            };
            for (auto blk = doc.RootElement()->FirstChildElement(); blk;
            ...
            addFunction = [&](tinyxml2::XMLElement* blk) -> void {
              if (blk->Attribute("type", "function")) {
                file_js["functions"].push_back(
                    blk->FirstChildElement("field")->GetText());
              }
              if (auto next = blk->FirstChildElement("next"))
                addFunction(next->FirstChildElement());
            };
            for (auto blk = doc.RootElement()->FirstChildElement(); blk;
                 blk = blk->NextSiblingElement()) {
              addFunction(blk);
...
auto createProgram(std::string pro_js_str) -> std::string {
  ...
  std::filesystem::path dat = program_path / (pro_name + ".dat"),
                        pro = program_path / (pro_name + ".pro");
  ...
  pro_file_js["path"] =
      "/program/" + dir.path().filename().string() + "/" + pro_name + ".pro";
  pro_file_js["content"] =
  ...
  dat_file_js["path"] =
      "/program/" + dir.path().filename().string() + "/" + pro_name + ".dat";
  dat_file_js["content"] =
      "<xml xmlns=\"https://developers.google.com/blockly/xml\"></xml>";
  ...
  aris_file_js["path"] =
      "/program/" + dir.path().filename().string() + "/" + pro_name + ".aris";
...
auto updateProgram(std::string pro_name, std::string data) -> std::string {
  ...
  for (auto& file : js["files"]) {
    ...
    // LOG_INFO << "HTTP UPDATE PROGRAM FILE:" <<
    // file["name"].get<std::string>() << std::endl;

    std::fstream f(
        program_path / pro_name / "temp" / file["name"].get<std::string>(),
        std::ios::out | std::ios::trunc);
    f << file["content"].get<std::string>();
    f.close();

    if (auto ext =
            std::filesystem::path(file["name"].get<std::string>()).extension();
        ext != ".dat" && ext != ".pro")
      continue;

    tinyxml2::XMLDocument doc;
    if (doc.Parse(file["content"].get<std::string>().c_str())) {
      has_error = true;
      break;
    }
    ...
    addFunction = [&](tinyxml2::XMLElement* blk) -> void {
      if (blk->Attribute("type", "jointtarget_variable")) {
        file["jointtargets"].push_back(
            blk->FirstChildElement("field")->GetText());
      } else if (blk->Attribute("type", "robtarget_variable")) {
        file["robtargets"].push_back(
            blk->FirstChildElement("field")->GetText());
      } else if (blk->Attribute("type", "speed_variable")) {
  ...
  if (has_error == false) {
    ...
    for (auto& file : std::filesystem::directory_iterator(program_path /
                                                          pro_name / "temp")) {
      if (std::filesystem::is_regular_file(file)) {
        std::filesystem::copy_file(
            file.path(), program_path / pro_name / file.path().filename());
      }
    }
  } else {
...
auto renameProgram(std::string old_name, std::string new_name_js)
    -> std::string {
  if (!std::filesystem::exists(rootPath() / "robot/program" / old_name))
    return "";
  ...
  std::filesystem::rename(rootPath() / "robot/program" / old_name,
                          rootPath() / "robot/program" / new_name);
  std::filesystem::rename(
      rootPath() / "robot/program" / new_name / (old_name + ".pro"),
      rootPath() / "robot/program" / new_name / (new_name + ".pro"));
  std::filesystem::rename(
  ...
  std::filesystem::path dat = rootPath() / "robot/program" / new_name /
                              (new_name + ".dat"),
                        pro = rootPath() / "robot/program" / new_name /
                              (new_name + ".pro");
  ...
  pro_file_js["path"] = "/program/" + dir.path().filename().string() + "/" +
                        pro.filename().string();
  std::ifstream f(pro);
  std::string str((std::istreambuf_iterator<char>(f)),
                  std::istreambuf_iterator<char>());
  ...
  dat_file_js["path"] = "/program/" + dir.path().filename().string() + "/" +
                        dat.filename().string();
  f.close();
  f.open(dat);
...
auto postObjPicture(std::string str) -> std::string {
  ...
  auto data = str.substr(head_size + 4,
                         str.rfind("\r\n", str.size() - 5) - head_size - 4);

  std::fstream file;
  file.open(rootPath() / "detech" / (filename),
