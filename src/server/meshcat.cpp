#include "sire/server/meshcat.hpp"
...
#include "sire/server/meshcat_types.hpp"
...
std::string CreateCommand(const std::string& data) {
  return sire::core::string_format(
             R"""(fetch("data:application/octet-binary;base64,%s")
               .then(res => res.arrayBuffer())
               .then(buffer => viewer.handle_command_bytearray(new Uint8Array(buffer)));
           )""",
             sire::server::base64_helper::Encode(
...
class MeshcatShapeCalculator : public sire::geometry::ShapeCalculator {
 public:
  ARIS_DELETE_BIG_FOUR(MeshcatShapeCalculator);

  explicit MeshcatShapeCalculator(uuids::uuid_random_generator* uuid_generator)
      : uuid_generator_(uuid_generator) {
    SIRE_DEMAND(uuid_generator != nullptr);
  }

  ~MeshcatShapeCalculator() = default;

  using sire::geometry::ShapeCalculator::ImplementGeometry;

  template <typename T>
  void ImplementMesh(const T& mesh, void* data) {
    SIRE_DEMAND(data != nullptr);
  ...
  void ImplementGeometry(const sire::geometry::BoxShape& box,
                         void* data) override {
    SIRE_DEMAND(data != nullptr);
    auto& lumped = *static_cast<internal::LumpedObjectData*>(data);
  ...
  void ImplementGeometry(const sire::geometry::SphereShape& sphere,
                         void* data) override {
    SIRE_DEMAND(data != nullptr);
    auto& lumped = *static_cast<internal::LumpedObjectData*>(data);
    lumped.object = internal::MeshData();
  ...
  //   // origin.  A cone is just a cylinder with radiusBottom=0.  So we
  //   transform
...
class Meshcat::Impl {
 public:
  ...
  explicit Impl(const MeshcatParams& params)
      : prefix_("/sire"),
        ...
        params_(params) {
    SIRE_ASSERT(params.port.value_or(7000) >= 1024);
    ...
    const std::string url =
        sire::core::string_format(params.web_url_pattern.c_str(), "localhost",
                                  params.port.value_or(5867));
    ...
    // websocket_thread_ =
    //     std::thread(&Impl::WrappedWebSocketMain, this,
  ...
  std::string web_url() const {
    ...
    //                    fmt::arg("host", display_host), fmt::arg("port",
    //                    port_));
  }
  ...
  std::string ws_url() const {
    ...
    SIRE_DEMAND(http_url.substr(0, 4) == "http");
  ...
  void Flush() const {
    ...
    if (main_backpressure > 0) {
      // drake::log()->warn(
      //     "Meshcat::Flush() reached an iteration limit before the buffer
      //     could " "be completely flushed.");
    }
  }

  // This function is public via the PIMPL.
  void SetObject(std::string_view path, const sire::geometry::ShapeBase& shape,
                 const Rgba& rgba) {
    // DRAKE_DEMAND(IsThread(main_thread_id_));
    ...
    MeshcatShapeCalculator calculator(&uuid_generator);
    shape.Reify(&calculator, &data.object);
    ...
    if (std::holds_alternative<internal::MeshData>(data.object.object)) {
      ...
      SIRE_DEMAND(data.object.geometry != nullptr);
  ...
  // This function is public via the PIMPL.
  // void SetObject(std::string_view path, const TriangleSurfaceMesh<double>&
  // mesh,
  //                const Rgba& rgba, bool wireframe, double
  //                wireframe_line_width, SideOfFaceToRender side) {
  //   DRAKE_DEMAND(IsThread(main_thread_id_));
  ...
  void SetLineSegments(std::string_view path,
                       const std::vector<std::array<double, 3>>& start,
                       //  const Eigen::Ref<const Eigen::Matrix3Xd>& start,
                       const std::vector<std::array<double, 3>>& end,
                       double line_width, const Rgba& rgba) {
    // DRAKE_DEMAND(IsThread(main_thread_id_));
    // DRAKE_THROW_UNLESS(start.cols() == end.cols());
    // // The LineSegments loader in three.js take the same data structure as
    // Line,
    // // but takes every consecutive pair of vertices as a (start, end).
    // Eigen::Matrix<double, 6, Eigen::Dynamic> vstack(6, start.cols());
    // vstack << start, end;
    // Eigen::Map<Eigen::Matrix3Xd> vertices(vstack.data(), 3, 2 *
    // start.cols()); const bool kLineSegments = true; SetLineImpl(path,
    // vertices, line_width, rgba, kLineSegments);
  }
  ...
  // This function is public via the PIMPL.
  void SetTransform(std::string_view path, double* X_ParentPath) {
    // DRAKE_DEMAND(IsThread(main_thread_id_));
    // SetTransform(path, X_ParentPath.GetAsMatrix4());
  }

  // This function is public via the PIMPL.
  void SetTransform(std::string_view path, const double* matrix) {
    // DRAKE_DEMAND(IsThread(main_thread_id_));
  ...
  // This function is public via the PIMPL.
  void Set2dRenderMode(const double* X_WC = sire::default_pm, double xmin,
  ...
  void AddButton(std::string name, std::string keycode) {
    ...
    //             fmt::format("Meshcat already has a slider named {}.",
    //             data.name));
    //       }
    ...
    //               "Meshcat already has a button named `{}`, but the
    //               previously " "assigned keycode `{}` does not match the
    //               current keycode `{}`. " "To re-assign the keycode, you must
    //               first delete the button.", data.name,
    //               iter->second.keycode1, data.keycode1));
    //         }
    //       }
    //       buttons_[data.name] = data;
    //       DRAKE_DEMAND(controls_.size() == (buttons_.size() +
    //       sliders_.size()));
    //     }
    ...
    //       std::stringstream message_stream;
    //       msgpack::pack(message_stream, data);
    //       app_->publish("all", message_stream.str(), uWS::OpCode::BINARY,
    //       false);
    //     });
  }
  ...
  void AddSlider(std::string name, double min, double max, double step,
    ...
    //         throw std::logic_error(
    //             fmt::format("Meshcat already has a button named {}.",
    //             data.name));
    //       }
    //       if (sliders_.find(data.name) != sliders_.end()) {
    //         throw std::logic_error(
  ...
  void DeleteAddedControls() {
    // DRAKE_DEMAND(IsThread(main_thread_id_));
    // // We copy the data structures so that the main thread can iterate
    // through
  ...
  std::string StaticHtml() {
    ...
    // // Replace the javascript code in the original html file which connects
    // via
  ...
  void InjectWebsocketThreadFault(int fault_number) {
    ...
    //     Defer([this]() {
    //       DRAKE_DEMAND(IsThread(websocket_thread_id_));
    //       throw std::runtime_error("InjectWebsocketThreadFault during
    //       defer");
    //     });
    //     return;
    //   }
 ...
 private:
  ...
  void WebSocketMain(std::promise<std::tuple<int, bool>> app_promise,
    ...
    //   // until we know that the main thread is no longer adding more
    //   callbacks.
    ...
    // // This canonicalization of the bind_host is currently redundant with
    // what
    // // uWebSockets already implements, but we'll keep it here anyway, to
    // defend
    ...
    // // Set maxBackpressure = 0 so that uWS does *not* drop any messages due
    // to
    // // back pressure.
    ...
    //     // However, when exiting via exception it only places the socket
    //     memory
    //     // onto a close-list instead of freeing it. To avoid heap leaks,
    //     we'll
    //     // manually free the memory here using an internal helper function.
    //     // TODO(jwnimmer-tri) Probably uWS::LoopCleaner::~LoopCleaner should
    //     be
    //     // doing this? Submit a ticket with upstream to find the correct
    //     answer. us_internal_free_closed_sockets(
  ...
  void HandleSocketOpen(WebSocket* ws) {
    ...
    //   throw std::runtime_error("InjectWebsocketThreadFault during socket
    //   open");
    // }
  }
  ...
  void HandleMessage(WebSocket* ws, std::string_view message) {
    ...
    //   // This violates the style guide, but msgpack does not provide any
    //   other
  ...
  void Shutdown() {
    ...
    // // Close any existing connections. Calling ws->close() erases the
    // WebSocket
    // // from websockets_, so we need to advance the iterator beforehand
    // (#15821). auto iter = websockets_.begin(); while (iter !=
    // websockets_.end()) {
    //   WebSocket* ws = *iter++;
    //   ws->close();
    // }
...
namespace {
sire::server::MeshcatParams MakeMeshcatParamsPortOnly(std::optional<int> port) {
...
Meshcat::~Meshcat() { delete static_cast<Impl*>(impl_); }
...
std::string Meshcat::web_url() const { return impl().web_url(); }

int Meshcat::port() const { return impl().port(); }

std::string Meshcat::ws_url() const { return impl().ws_url(); }

int Meshcat::GetNumActiveConnections() const {
  return impl().GetNumActiveConnections();
}

void Meshcat::Flush() const { impl().Flush(); }

void Meshcat::SetObject(std::string_view path,
                        const sire::geometry::ShapeBase& shape,
                        const Rgba& rgba) {
  impl().SetObject(path, shape, rgba);
}

// void Meshcat::SetObject(std::string_view path,
//                         const perception::PointCloud& cloud, double
//                         point_size, const Rgba& rgba) {
...
void Meshcat::SetLineSegments(
    std::string_view path, const std::vector<std::array<double, 3>>& start,
    //  const Eigen::Ref<const Eigen::Matrix3Xd>& start,
    const std::vector<std::array<double, 3>>& end,
...
void Meshcat::SetTriangleMesh(
    std::string_view path, const std::vector<std::array<double, 3>>& vertices,
    //  const Eigen::Ref<const Eigen::Matrix3Xd>& vertices,
    const std::vector<std::array<int, 3>>& faces, const Rgba& rgba,
...
void Meshcat::SetTriangleColorMesh(
    std::string_view
        path,  // const Eigen::Ref<const Eigen::Matrix3Xd>& vertices,
    const std::vector<std::array<double, 3>>& vertices,
    // const Eigen::Ref<const Eigen::Matrix3Xi>& faces,
    const std::vector<std::array<int, 3>>& faces,
    const std::vector<std::array<double, 3>>& colors, bool wireframe,
    double wireframe_line_width, SideOfFaceToRender side) {
...
void Meshcat::PlotSurface(std::string_view path,
  ...
  //       faces.col(count++) << ids(i + 1, j), ids(i + 1, j + 1), ids(i, j +
  //       1);
  //     }
  //   }
...
void Meshcat::SetTransform(std::string_view path, const double* X_ParentPath,
...
void Meshcat::SetTransform(std::string_view path, const double* matrix) {
  impl().SetTransform(path, matrix);
}

void Meshcat::Delete(std::string_view path) { impl().Delete(path); }

void Meshcat::SetRealtimeRate(double rate) { impl().SetRealtimeRate(rate); }
...
void Meshcat::Set2dRenderMode(const double* X_WC, double xmin, double xmax,
                              double ymin, double ymax) {
  impl().Set2dRenderMode(X_WC, xmin, xmax, ymin, ymax);
}

void Meshcat::ResetRenderMode() { impl().ResetRenderMode(); }
...
void Meshcat::DeleteAddedControls() { impl().DeleteAddedControls(); }

Meshcat::Gamepad Meshcat::GetGamepad() const { return impl().GetGamepad(); }

std::string Meshcat::StaticHtml() { return impl().StaticHtml(); }
