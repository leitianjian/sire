...
string NiceTypeName::Canonicalize(const string& demangled) {
  ...
  // static const variable lifetime from stackoverflow
  // https://stackoverflow.com/a/246568/8462770
  static const std::array<SPair, 14> subs{{
...
string NiceTypeName::RemoveNamespaces(const string& canonical) {
  ...
  const std::string no_namespace = std::regex_replace(canonical, regex, "");
