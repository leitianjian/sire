#ifndef SIRE_MESHCAT_HPP_
#define SIRE_MESHCAT_HPP_
...
struct MeshcatParams {
  ...
  - `{host}` will be substituted with this params structure's `host` field, or
    ...
    "all interfaces".
  */
  std::string web_url_pattern{"http://%s:%i"};
...
class Meshcat {
 public:
  ...
  ~Meshcat();
  SIRE_DECLARE_NO_COPY_NO_MOVE(Meshcat)
  ...
  */
  ...
  void Set2dRenderMode(const double* X_WC = sire::default_pm, double xmin = -1,
                       double xmax = 1, double ymin = -1, double ymax = 1);
  ...
  */
  void SetTransform(
      std::string_view path, const double* X_ParentPath,
  ...
  */
  ...
  //   /* (Internal use for unit testing only) Causes the websocket worker
  //   thread to exit with an error, which will spit out an exception from the
  //   next Meshcat main thread function that gets called. The fault_number
  //   selects which fault to inject, between 0 and kMaxFaultNumber inclusive;
  //   refer to the implementation for details on meaning of each number. */
