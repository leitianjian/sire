...
class SIRE_API MakeBlockly {
 public:
  auto trim_left(std::string_view input) -> std::string_view {
    ...
    return point == std::string::npos ? std::string_view()
                                      : input.substr(point, std::string::npos);
  };

  auto parse_raw(std::string_view cmd_str)
    ...
    auto cut_str = [](std::string_view& input,
      ...
      input =
          point == std::string::npos ? std::string_view() : input.substr(point);
      return ret;
    };
    auto get_param_value = [&](std::string_view& cmd_str) -> std::string_view {
      int brace_num = 0;
      std::size_t i = 1;
      for (i = 1; i < cmd_str.size() &&
                  !(std::isspace(static_cast<unsigned char>(cmd_str[i])) &&
                    brace_num == 0);
           ++i) {
        switch (cmd_str[i]) {
          case '{':
    ...
    if (cmd = cut_str(cmd_str, " "); cmd.empty())
      THROW_FILE_LINE("invalid command string: please at least contain a word");
    ...
    for (; !cmd_str.empty();) {
      ...
      param_map.insert(make_pair(std::string_view(param_name),
  ...
  auto make(std::filesystem::path program) {
    ...
    //////////////////////////make var/////////////////////////////////
    {
      ...
      while (std::getline(pf, line, '\n')) {
        ...
        std::string data =
            std::string(trim_left(line.substr(line.find_first_of(":") + 1)));
        ...
        if (word == "var") {
          ...
          if (type == "robtarget") {
            ...
            auto mat = std::any_cast<aris::core::Matrix>(
                calculator.calculateExpression(value).second);

            auto names = {"x", "y", "z", "a", "b", "c"};
            double coe[] = {
                1000.0,         1000.0,         1000.0,
                180 / aris::PI, 180 / aris::PI, 180 / aris::PI,
            };
            ...
            auto names = {"percent", "linear", "angular", "exj_angular",
                          "exj_linear"};
    ...
    //////////////////////////make pro/////////////////////////////////
    {
      ...
      while (std::getline(pf, line, '\n')) {
        ...
        if (word == "main" || word == "function" || word == "if" ||
            word == "while") {
          ...
          if (word == "function") {
            std::string func_name =
                std::string(trim_left(data.substr(word.size())));
            ...
            auto [name, value] =
                this->cal.calculateExpression(data.substr(word.size()));
            auto v = std::any_cast<tinyxml2::XMLElement*>(value);
            condition->InsertFirstChild(
                std::any_cast<tinyxml2::XMLElement*>(value));
          ...
          lbp = state;
        } else if (word == "endmain" || word == "endfunction" ||
                   word == "endif" || word == "endwhile") {
          lbp = stack.back();
          stack.pop_back();
...
auto SIRE_API setRootPath(std::filesystem::path path) -> void;

auto SIRE_API fetchInterfaceConfig() -> std::string;
auto SIRE_API updateDashboard(std::string dash_id, std::string js_str)
    -> std::string;
auto SIRE_API createCell(std::string dash_id, std::string cell) -> std::string;
auto SIRE_API deleteCell(std::string dash_id, std::string cell_id)
    -> std::string;

auto SIRE_API fetchPrograms() -> std::string;
auto SIRE_API createProgram(std::string pro_name) -> std::string;
auto SIRE_API updateProgram(std::string js_str, std::string data)
    -> std::string;
auto SIRE_API deleteProgram(std::string pro_name) -> std::string;
auto SIRE_API renameProgram(std::string old_name, std::string new_name_js)
    -> std::string;

auto SIRE_API fetchConfigXml() -> std::string;
auto SIRE_API fetchESIPath() -> std::string;

auto SIRE_API fetchObjPictureList() -> std::string;
auto SIRE_API postObjPicture(std::string str) -> std::string;
}  // namespace sire::server
