#ifndef SIRE_PS_VS_SOLVER3_HPP_
#define SIRE_PS_VS_SOLVER3_HPP_
...
namespace contact::ps_vs_solver3 {
...
class SIRE_API PsVsSolver3 : public ContactSolver {
 public:
  PsVsSolver3();
  virtual ~PsVsSolver3();
  SIRE_DECLARE_MOVE_CTOR(PsVsSolver3);
  ...
  /// Applies resolved forces to model's forcePool via cptGlbContactWrench.
  /// Returns the actual dt to use for integration (may be < suggest_dt if
  /// contact ends mid-interval).
  /// @param[in]  model           Current kinematic state.
  /// @param[in]  engine           Physics engine (for material / geometry).
  /// @param[in]  penetration_pairs  Raw collision-detection pairs.
  /// @param[out] T_C_vec          Contact-frame transforms (filled internally).
  /// @param[out] contact_info     Per-contact force details (filled).
  /// @param[in]  suggest_dt       Nominal dt to try.
  /// @return     Actual dt ≤ suggest_dt (or suggest_dt if no contacts).
