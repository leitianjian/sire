#ifndef SIRE_DEPRECATED_HPP_
#define SIRE_DEPRECATED_HPP_
...
/** Use `SIRE_DEPRECATED("removal_date", "message")` to discourage use of
...
displaying the given message, preceded by "SIRE DEPRECATED: ". The Doxygen API
...
Try to keep the date string immediately after the SIRE_DEPRECATED macro name,
even if the message itself must be wrapped to a new line:
@code
  SIRE_DEPRECATED("2038-01-19",
...
Sample uses: @code
  ...
  SIRE_DEPRECATED("2038-01-19", "f() is slow; use g() instead.")
  int f(int arg);

  // Attribute comes *after* struct, class, enum keyword.
  class SIRE_DEPRECATED("2038-01-19", "Use MyNewClass instead.")
  MyClass {
  };

  // Type alias goes before the '='.
  using OldType
      SIRE_DEPRECATED("2038-01-19", "Use NewType instead.")
      = NewType;
@endcode
*/
#define SIRE_DEPRECATED(removal_date, message)

#else  // SIRE_DOXYGEN_CXX

#define SIRE_DEPRECATED(removal_date, message)                   \
  [[deprecated("\nSIRE DEPRECATED: " message                     \
               "\nThe deprecated code will be removed from Sire" \
