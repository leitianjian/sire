...
def _center_camera_on_robot(viewer, env, distance=3.0, elevation=-20.0, azimuth=135.0):
...
def _apply_fixed_start_state(env):
...
def _deployment_step(env, actions: torch.Tensor):
...
def main():
    ...
    with mujoco.viewer.launch_passive(env.models[0], env.datas[0]) as viewer:
        _center_camera_on_robot(viewer, env)
